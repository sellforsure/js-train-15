...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const randomSymb = characters[randomIndex];
    password + randomSymb;
  }
...
function sumPositiveNumbers(numbers) {
  ...
  for (let value = 0; value < numbers.length; value++) {
    if (Math.sign(numbers[value]) === 1) {
      sumNumbers += numbers[value];
    }
  }
...
function getFractionalPart(num) {
  ...
  const numTrunc = Math.trunc(num);
  const diff = num - numTrunc;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sumInv = principal * Math.pow(1 + interestRate, years);

  return Math.round(sumInv);
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((accumulator, value) => {
    return accumulator + value.price;
  }, 0);

  const froundTotalPrice = Math.fround(totalPrice);
  const froundMaxPrice = Math.fround(maxPrice);

  return froundTotalPrice > froundMaxPrice;
}
